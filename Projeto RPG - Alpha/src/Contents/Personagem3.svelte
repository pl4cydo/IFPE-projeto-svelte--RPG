<script type="text/javascript">
	...
	var audioBatalha3 = new Audio('./music/batalha3.mp3');
	audioBatalha3.play();
	...
	function paoleite() {
		if (contadorCuraMessiah < 2) {
			...
			if ((cura + $Messiah.vidaCarlos) > 194) {
				...
				$Messiah.vidaCarlos += cura
			}
			movimentoCuraMessiah()
			setTimeout(function(){
				inf3.innerHTML = $Messiah.nome + " curou 50% do HP"
				barraVidaBus3.style.width = $Messiah.vidaCarlos + "px"
			},2000);
		} else {
			inf3.innerHTML = "Enjoou do Leite Condensado"
		}
		setTimeout(function(){
    		inf3.innerHTML = "Fim do turno"
		},6000);	
	...
	function movimentoCuraMessiah() {
		personagemBus3.style.top = "38px"
		setTimeout(function(){
